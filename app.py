...
def artificial_bee_colony(max_iterations, colony_size, dimensions):
    # Initialize population in larger range
    ...
    for iteration in range(max_iterations):
        # Employed bees phase (modified exploration)
        for i in range(colony_size):
            k = np.random.randint(0, colony_size)
            while k == i:
                k = np.random.randint(0, colony_size)
            phi = np.random.uniform(-1.5, 1.5, dimensions)  # Larger mutation
            new_solution = population[i] + phi * (population[i] - population[k])
            # Keep solutions in bounds
        ...
        # Rest of the ABC algorithm remains the same...
        # (keep your existing onlooker and scout phases)

        # Scout bees phase
        for i in range(colony_size):
            if fitness[i] > np.percentile(fitness, 90):  # Replace worst solutions
    ...
    def generate():
        ...
        for iteration, best_solution, best_fitness, mean_fitness in abc_gen:
            ...
            data = {
                ...
                'computation_time': elapsed_time  # Add computation time
            }
